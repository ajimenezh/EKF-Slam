...
void PngImage::DrawSegment(const Point& l, const Point& rp, int size, int r, int g, int b) {
	for (int i = 0; i <= size; i++) {
		double x = l.x + (rp.x - l.x) * (1.0 * i / size);
		double y = l.y + (rp.y - l.y) * (1.0 * i / size);
		FillPoints({ { x, y } }, 1, r, g, b);
	}
}
...
void PngImage::DrawCircle(const Point& c, double rad, int r, int g, int b) {
	int max = 100 * rad;
	double pi = acos(-1.0);
	for (int i = 0; i <= max; i++) {
		double x = c.x + rad * cos(2.0 * pi * i / max);
		double y = c.y + rad * sin(2.0 * pi * i / max);
		FillPoints({ { x, y } }, 1, r, g, b);
	}
}
